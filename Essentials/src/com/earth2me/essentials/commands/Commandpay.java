...
public class Commandpay extends EssentialsLoopCommand {
    ...
    public void run(final Server server, final User user, final String commandLabel, final String[] args) throws Exception {
        ...
        BigDecimal amount = BigDecimal.valueOf(Double.parseDouble(stringAmount) / 100d);

        if (args.length == 3) {
            String extra = args[2];
            if (extra.equalsIgnoreCase("galleon") || extra.equalsIgnoreCase("galleons")) {
                amount = BigDecimal.valueOf(Double.parseDouble(stringAmount) * 17d * 29d / 100d);
            } else if (extra.equalsIgnoreCase("sickle") || extra.equalsIgnoreCase("sickles")) {
                amount = BigDecimal.valueOf(Double.parseDouble(stringAmount) * 29d / 100d);
            } else if (!(extra.equalsIgnoreCase("knut") || extra.equalsIgnoreCase("knuts"))) {
                throw new NotEnoughArgumentsException();
            }
        }
        ...
        loopOnlinePlayersConsumer(server, user.getSource(), false, user.isAuthorized("essentials.pay.multiple"), args[0], player -> {
            try {
                ...
                if (user.isPromptingPayConfirm() && !finalAmount.equals(user.getConfirmingPayments().get(player))) { // checks if exists and if command needs to be repeated.
                    ...
                    user.getConfirmingPayments().put(player, finalAmount);
                    return;
                }
                user.payUser(player, finalAmount, UserBalanceUpdateEvent.Cause.COMMAND_PAY);
                user.getConfirmingPayments().remove(player);
                Trade.log("Command", "Pay", "Player", user.getName(), new Trade(finalAmount, ess), player.getName(), new Trade(finalAmount, ess), user.getLocation(), ess);
            } catch (MaxMoneyException ex) {
                user.sendMessage(tl("maxMoney"));
                try {
                    user.setMoney(user.getMoney().add(finalAmount));
